...
def build_local_solution_set(prior):
    ...
    for s in straights:
        for l1 in letters:
            for l2 in letters:
                for l3 in letters:
                    sset.append(prior[:2] + [l1] + [l2] + [l3] + s)
                    sset.append(prior[:2] + [l1] + [l2] + s + [l3])
                    sset.append(prior[:2] + [l1] + s + [l2] + [l3])
                    sset.append(prior[:2] + s + [l1] + [l2] + [l3])
...
def password(in_file):
    ...
    with open(in_file) as f:
        ...
        next_password2 = build_local_solution_set(password)

    return next_password2
